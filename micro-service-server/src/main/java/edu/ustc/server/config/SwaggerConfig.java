...
public class SwaggerConfig {
	...
	public Docket swaggerSpringBootPlugin() {
		...
		return new Docket(DocumentationType.SWAGGER_2).groupName("micro-service-server").useDefaultResponseMessages(false)
			.forCodeGeneration(true).pathMapping("/").select().apis(RequestHandlerSelectors.any()).paths(PathSelectors.regex("/api/.*"))
			.build().apiInfo(apiInfo());
	}
	
	private ApiInfo apiInfo() {
		return new ApiInfo("micro-service-server api", null, "1.0", null, null, null, null);
	}
}
